...
import { archiveListManager, unarchiveDossierManager } from "../../../services/manager_services/dossiersService";
...
const ArchiveManager = () => {
  ...
  const fetchArchives = async () => {
    try {
      const data = await archiveListManager();
      setArchives(data);
    } catch (error) {
  ...
  const handleUnarchive = async (id) => {
    const success = await unarchiveDossierManager(id);
    if (success) {
...
const statusCount = filteredArchives.reduce((acc, dossier) => {
